...
| **数据层**     | SQLite WAL 模式、内容/分析/翻译任务持久化、重复数据迁移      | 正式 Alembic 迁移体系     |
| **Worker**     | OpenAI/DeepSeek JSON 适配、截断精简重试、原子任务领取、低价时段调度、三阶段分析、Markdown 分块翻译与断点续跑、失败隔离 | 任务级退避重试、超时恢复  |
| **Web**        | 登录改密、Key 管理、整理时段开关、Inbox 分类（用户修正优先）、阅读建议、Markdown 阅读/源码视图、中英对照、初始问卷、评测开关、阅读后反馈、四级校准矩阵、规则候选确认 | AI 差异解释、忽略内容抽检 |
