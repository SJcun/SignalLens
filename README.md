...
| **后端 API**   | 单用户登录与改密、可撤销会话、最小权限插件 Key、内容采集与去重、分析状态管理、每日整理时段与立即整理、英文翻译任务、显式用户画像、人工反馈快照、校准统计 | 画像修改建议              |
| **数据层**     | SQLite WAL 模式、内容/分析/翻译任务持久化、重复数据迁移、分析快照正文哈希与来源章节清单 | 正式 Alembic 迁移体系     |
| **Worker**     | OpenAI/DeepSeek JSON 适配、截断精简重试、原子任务领取、低价时段调度、三阶段分析、Markdown 分块翻译与断点续跑、失败隔离、模型调用前生成来源章节清单 | 任务级退避重试、超时恢复  |
| **Web**        | 登录改密、Key 管理、整理时段开关、Inbox 分类（用户修正优先）、阅读建议、Markdown 阅读/源码视图、中英对照、初始问卷、评测开关、阅读后反馈、四级校准矩阵、规则候选确认、选择性阅读顺序式引导阅读流 | AI 差异解释、忽略内容抽检 |
...
- **引导阅读流**：`selective_read` 的章节动作携带 `section_ref`，Web 按来源顺序原位展示原文、摘要与折叠卡片；引用缺失、重复或正文哈希变化时整体退回完整原文，不展示部分引导流；
- **翻译**：用户主动触发，不受时段限制。按 Markdown 块分批翻译并保存进度，代码/图片/链接保持原样。正文变化后旧译文自动失效；
- **失败处理**：单任务失败标记为 `failed`，不终止 Worker，可在详情页重试；
- **画像**：保存后用于后续新任务，不自动重算历史；
