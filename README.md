...
系统按单用户设计：唯一账户为 `admin`，不提供注册接口。**不存在任何出厂默认口令**，首次凭据与强制改密机制如下。

### 初始密码是随机生成的，不是默认口令

- 每次启动都会检查 `admin_users` 表，仅当不存在 `admin` 账户时才自动创建；重复启动不会重新生成或重置密码；
- 初始密码由 `secrets.token_urlsafe(18)` 随机生成（约 24 字符高熵），不存在 `admin/admin123` 之类的固定弱口令，也无法从代码或文档推断；
- 密码在数据库中只保存 **scrypt 哈希**（16 字节随机盐 + 64 字节派生值，`N=2^14, r=8, p=1`），任何位置都不存明文；登录校验使用 `secrets.compare_digest` 常量时间比较，避免时序侧信道。

### 初始密码只在本机可见

- 一次性凭据写入 `SIGNALLENS_BOOTSTRAP_PASSWORD_FILE` 指向的文件（默认 `data/initial-admin-password.txt`，Docker 部署为容器挂载卷的 `/data/initial-admin-password.txt`），内容为 `username=admin` 与 `password=...` 两行；
- 写入采用临时文件原子替换，并在支持的平台限制为当前用户可读写（0600）；
- 该文件已被 Git 忽略，**不会进入版本库或任何历史提交**；凭据先落盘再创建账户，避免文件写入失败后无法登录。

### 首次登录后强制改密

- 初始账户带有“必须改密”标记，登录响应会携带该标记；**用初始密码登录 Web 后会强制进入“账户安全”页**，完成改密前不会跳转到其他页面，并持续显示改密提醒横幅；
- 修改密码必须提供当前密码，新密码不能与当前密码相同，并以 scrypt 哈希重新落库；
- 改密成功瞬间会同时执行：**撤销全部 Web 会话**（所有已登录设备立即失效）、清除“必须改密”标记、**删除初始密码文件**——一次性初始凭据彻底作废，即使该文件此前被复制或泄露也无法再次登录；
- 此后系统与常规密码体系无异：每次登录使用你设置的新密码，密码永不以明文形式出现在磁盘或数据库中。

### 会话与插件 Key 同样遵循最小暴露原则

- Web 会话令牌为随机 Bearer Token（`secrets.token_urlsafe(32)`），数据库仅保存 SHA-256 摘要，可服务端撤销，默认 30 天过期；
- 插件使用独立的 `sk-sl-` Key（数据库同样只存哈希），仅允许调用采集提交接口，不接触 admin 密码与登录会话。
